...
export default function CircularDiagram({ language }: { language: "ru" | "uk" }) {
  ...
  const model = useMemo(() => {
    ...
    const at = (r: number) => Number.isFinite(r)
      ? add(i0, mul(sub(ik, i0), div(zout, add(zout, polar(r, phi)))))
      : i0;
    const points = [at(0)];
    ...
    const extent = Math.max(1, ...points.flatMap(p => [Math.abs(p.re), Math.abs(p.im)]), abs(ik)) * 1.18;
    const scale = Math.min(612 / (extent * 2), 412 / (extent * 2));
    const xy = (p: C) => ({ x: 360 + p.re * scale, y: 260 - p.im * scale });
    const path = points.map((p, n) => {
      const q = xy(p);
      return `${n ? "L" : "M"}${q.x.toFixed(2)},${q.y.toFixed(2)}`;
    }).join(" ");
  ...
  const vectors = [
    ...
    { id: "active", label: "I(R)", value: model.current },
  ];
  ...
  return <section className="circle-lab">
    <div className="circle-lab-heading">
      <div>
        <span className="section-kicker">{ru ? "КРУГОВАЯ ДИАГРАММА" : "КОЛОВА ДІАГРАМА"}</span>
        <h2>{ru ? "Годограф тока четырёхполюсника" : "Годограф струму чотириполюсника"}</h2>
      </div>
    ...
    <div className="circle-lab-layout">
      <div className="circle-controls">
        <Group title={ru ? "Ток холостого хода I₀" : "Струм холостого ходу I₀"}>
          <Field label="Модуль, А" value={i0m} min={0} step={.1} set={setI0m}/>
          <Field label={ru ? "Угол, °" : "Кут, °"} value={i0a} step={1} set={setI0a}/>
        </Group>
        <Group title={ru ? "Ток короткого замыкания Iк" : "Струм короткого замикання Iк"}>
          <Field label="Модуль, А" value={ikm} min={0} step={.1} set={setIkm}/>
          <Field label={ru ? "Угол, °" : "Кут, °"} value={ika} step={1} set={setIka}/>
        </Group>
        <Group title={ru ? "Выход и нагрузка" : "Вихід і навантаження"}>
          <Field label={ru ? "|Zвых|, Ом" : "|Zвих|, Ом"} value={zm} min={.01} step={.1} set={setZm}/>
          <Field label={ru ? "∠Zвых, °" : "∠Zвих, °"} value={za} step={1} set={setZa}/>
          <Field label={ru ? "Угол нагрузки φ, °" : "Кут навантаження φ, °"} value={phi} step={1} set={setPhi}/>
        </Group>
        <label className="resistance-control">
          <span>{ru ? "Сопротивление нагрузки R" : "Опір навантаження R"}<strong>{Number.isFinite(model.resistance) ? `${fmt(model.resistance)} Ом` : "∞"}</strong></span>
          <input type="range" min="0" max="100" step=".2" value={position} onChange={e => setPosition(Number(e.target.value))}/>
          <small><span>0</span><span>|Z|</span><span>∞</span></small>
        </label>
      </div>
      <div className="circle-plot">
        <svg viewBox="0 0 720 520" role="img" aria-label={ru ? "Круговая диаграмма токов" : "Колова діаграма струмів"}>
          ...
          <g className="plot-grid">{ticks.map(t => {
            const x = model.xy({ re: t, im: 0 }).x, y = model.xy({ re: 0, im: t }).y;
            return <g key={t}><line x1={x} y1="25" x2={x} y2="495"/><line x1="25" y1={y} x2="695" y2={y}/>{t !== 0 && <><text x={x + 5} y="277">{t}</text><text x="367" y={y - 6}>{t}</text></>}</g>;
          })}</g>
          ...
          {vectors.map(v => {
            const p = model.xy(v.value);
            return <g className={`current-vector ${v.id}`} key={v.id}><line x1="360" y1="260" x2={p.x} y2={p.y} markerEnd={`url(#arrow-${v.id})`}/><circle cx={p.x} cy={p.y} r={v.id === "active" ? 6 : 4}/><text x={p.x + 10} y={p.y - 10}>{v.label}</text></g>;
          })}
        </svg>
        <div className="circle-readout" aria-live="polite">{vectors.map(v => <span key={v.id}><b>{v.label}</b> = {fmt(abs(v.value))}∠{fmt(arg(v.value), 1)}° A</span>)}</div>
      </div>
    </div>
  </section>;
}
