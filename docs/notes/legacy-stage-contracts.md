...
2. Прочитай единственный selector в этом файле и только соответствующий ему
   stage record, затем `specs/README.md`, затрагиваемую SPEC и нужный участок
   `docs/ROADMAP.md`.
3. Подчиняйся status, `NEXT`, blockers и dependency DAG выбранного record. Если
   status `blocked`, сообщи blockers и остановись: не запускай stage и не
   переходи к downstream stage автоматически.
4. `OPTIONAL` не считай утверждённым требованием; смена selector/`NEXT`
   выполняется только в этом файле после подтверждённого изменения состояния.
...
4. Используй унаследованный `$plan-stage` только когда отдельное планирование
   действительно нужно; результат сохраняй только в выбранном stage record.
...
Результат P-02: выбранный stage record содержит конечный объём и acceptance
criteria без отдельного plan/status owner.
...
1. Выполни проверки из выбранного stage record и затронутой SPEC.
...
1. Обнови только документы, факты в которых изменились: выбранный
   record/selector в `prompts/STAGES.md`, `ROADMAP`, а при необходимости SPEC,
   architecture, decisions, design или security.
2. Повышай lifecycle/evidence только до подтверждённого уровня; зафиксируй один
   dependency-safe `NEXT` либо точный blocker в canonical record.
...
- Если выбранный stage record уже выполнен по коду, сначала синхронизируй его
  lifecycle/evidence, а не повторяй реализацию.
