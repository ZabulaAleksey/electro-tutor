...
| `PLAT-003` | application profiles отделены от provider identity/authority; Student и Tutor — composable private personas | `profiles-capabilities-audit.spec.md`, ADR-023, `ARCHITECTURE.md` | `ET-09.4a..e`: ordered audit→Account→grant→profile→HTTP→E2E evidence | audit + Account + grant/evaluator validated; profiles planned |
| `AUTHZ-001..003` | server-side matrix вычисляет capabilities; self/client/foreign escalation denied; critical authority/profile create атомарны с durable audit | `profiles-capabilities-audit.spec.md`, ADR-023, `SECURITY.md`, `API.md` | `ET-09.4b..e`: policy matrix, real DB atomicity/ownership, two-user Keycloak/API/browser E2E | grant/evaluator and authority audit validated; profile/HTTP/E2E pending |
...
| `PCA-GRANT-001..004` | trusted account grant/internal issuer, evaluator and idempotency | `profiles-capabilities-audit.spec.md`, ADR-023, `SECURITY.md`, `DATA_MODEL.md` | `ET-09.4b`: revision `20260909_0007`, 85 fast + 39 real PostgreSQL tests; `c..e`: profile consumption and complete AUTHZ E2E | grant/evaluator implemented; validated locally |
...
`ET-09.2`, ET-09.3 DEV auth, `ET-09.4a` audit foundation, `ET-09.4b0` internal
Account boundary и `ET-09.4b` trusted grant/evaluator подтверждены local
evidence. Whole `ET-09.4` partial: `ET-09.4c..e` остаются planned. Production
